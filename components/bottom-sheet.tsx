  ...
  return (
    <>
      ...
      <div className="relative">
        <Button
          className="bg-[#34413D] text-black text-md text-white w-auto self-start mt-5 rounded-[40px] border-2 border-neutral-300 py-2"
          onClick={openSheet}
        >
          <Image src="/people.svg" alt="ученики" width={20} height={20} />
          <p>Назначить</p>
        </Button>
                  ...
                  <div className="w-full flex flex-col space-y-4">
                    {items.map((item) => (
                      <div key={item.id} className="flex flex-row justify-between items-center">
                        ...
                        <Button onClick={() => toggleChosen(item.id)}>{item.isChosen ? "Выбрано" : "Назначить"}</Button>
                      </div>
                    ))}
                  </div>
                </div>
